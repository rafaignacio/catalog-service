...
public class ItemRepository : BaseRepository, IItemRepository
{
    ...
    {
        var list = await QueryAsync<ItemModel>("SELECT Name, Description, Image, Category, Price, Amount FROM Item", token: token);
    ...
    public async Task<ItemModel?> GetByName(string name, CancellationToken token = default) =>
        (await QueryAsync<ItemModel>("SELECT Name, Description, Image, Category, Price, Amount FROM Item WHERE Name = @Name", new { Name = name }, token: token))?.FirstOrDefault();
